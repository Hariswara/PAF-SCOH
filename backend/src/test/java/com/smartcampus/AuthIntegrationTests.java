...
class AuthIntegrationTests {
    ...
    void pendingProfileUserCanAccessRegistrationButNotGreet() throws Exception {
        ...
        // Can access registration endpoint — security allows STATUS_PENDING_PROFILE,
        // so we get 400 (validation) rather than 403 (forbidden)
    ...
    void passkeyLoginStartShouldBePermittedWithoutAuth() throws Exception {
        // Login start must be accessible without a session (unauthenticated user)
    ...
    void passkeyRegisterStartShouldReturn403ForPendingProfileUser() throws Exception {
        // A user with PENDING_PROFILE status must not access passkey registration
        userRepository.save(new User(
